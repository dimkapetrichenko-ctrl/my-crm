...
def index():
    ...
    params = []
    if search_query:
        sql += " AND (LOWER(c.name) LIKE LOWER(%s) OR LOWER(c.country) LIKE LOWER(%s) OR LOWER(c.buyer_type) LIKE LOWER(%s))"
    ...
    sql += f" ORDER BY (CASE WHEN c.next_event_date = '{today_str}' THEN 0 ELSE 1 END), c.name ASC"
...
def add_client():
    ...
    if name:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO clients (name, country, buyer_type, interest_level, website, mayer_reg) VALUES (%s, %s, %s, %s, %s, %s)",
            (name, request.form.get('country', ''), request.form.get('buyer_type', ''), request.form.get('interest_level', 'не опрацьовано'), request.form.get('website', ''), request.form.get('mayer_reg', 'Ні'))
        )
        conn.commit()
        cursor.close()
        conn.close()
